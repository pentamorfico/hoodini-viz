...
function DataGridView({
  ...
  const columns = useMemo(() => {
    ...
    const zoomCol =
      activeKey === 'genes' || activeKey === 'baselines' || activeKey === 'treeMetadata'
        ? [
            {
              id: '__zoom',
              title: 'Zoom',
  ...
  const gridTheme = useMemo(() => {
    ...
    if (resolvedTheme === 'dark') {
      return {
        ...
        textHeader: '#e5e5e5',
        borderColor: '#2f2f2f',
        horizontalBorderColor: '#2f2f2f',
        headerBottomBorderColor: '#2f2f2f',
        accentColor: '#1e1e1e', // much darker grayscale for selection (dark theme)
        accentLight: '#3a3a3a',
        accentFg: '#f7f7f7',
      };
    }
    return {
      ...
      accentColor: '#b5b5b5', // much lighter grayscale for selection (light theme)
      accentLight: '#fafafa',
      accentFg: '#050505',
    };
  }, [resolvedTheme]);
  ...
  const triggerZoom = useCallback(
    (colIdx, rowIdx) => {
      ...
      if (activeKey === 'baselines' && typeof onZoomBaseline === 'function') {
        onZoomBaseline(rowObj);
        return true;
      }
    ...
    [columnsForGrid, rows, activeKey, onZoomGene, onZoomBaseline, onZoomTree]
  );
  ...
  const buildSelectionFromVisibility = useCallback(() => {
    ...
    const selRows = rows.reduce((acc, rowObj, idx) => {
      ...
      const visible = rowId ? !(activeVisibility.hiddenSet?.has(rowId)) : true;
      if (visible) {
        return acc.add(idx);
      }
      return acc;
    ...
    >
      ...
      <div style={{ flex: 1, minHeight: 0 }}>
        ...
        overscrollX={200}
          ...
          rowSelectionMode={activeVisibility ? 'multi' : 'multi'}
          ...
          onGridSelectionChange={(sel) => {
            ...
            const dataChanged = selectionRowCountRef.current !== rows.length;
            ...
            rows.forEach((rowObj, idx) => {
              ...
              if (!rowId) return;
              const desiredVisible = applyRows.hasIndex(idx);
              const currentVisible = !hiddenSet.has(rowId);
