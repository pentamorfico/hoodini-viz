...
const HoodiniViz = React.forwardRef<unknown, HoodiniVizProps>(({
  ...
  const treeLabelPadding = React.useMemo(() => {
    ...
    const baseTextPadding = maxLen * charWidth;
    
    // Add extra padding based on phylolabel size to avoid overlap with hoods
    ...
    // Extra padding proportional to label size (larger text needs more space)
    const sizeBasedPadding = scaledLabelSize * 5.0; // 2.0x size factor for better spacing
  ...
  const getTooltip = ({object, layer}) => {
    ...
    if (layer && layer.id === 'protein-polygons') {
      ...
      if (gA) {
        ...
        // Only show cluster if it exists and is different from gene name (avoid redundancy)
        if (clusterA && String(clusterA) !== String(geneNameA)) {
          entries.push(['source cluster', clusterA]);
        }
      ...
      if (gB) {
        ...
        // Only show cluster if it exists and is different from gene name (avoid redundancy)
        if (clusterB && String(clusterB) !== String(geneNameB)) {
          entries.push(['target cluster', clusterB]);
        }
  ...
  const legendEntries = React.useMemo(() => {
    ...
    if (sourceMap && sourceMap.size > 0) {
      ...
      for (const [k, color] of sourceMap.entries()) {
        ...
        if (typeof k === 'string') {
          const label = String(k).trim(); // Normalize label
          // Only add if we haven't seen this exact label before and label is not empty
          if (!seenLabels.has(label) && label.length > 0) {
            seenLabels.add(label);
            items.push({ label, color });
          }
        }
      }
      // Final deduplication pass in case map has duplicates
      const uniqueItems = [];
      const seenItemLabels = new Set();
      for (const item of items) {
        if (!seenItemLabels.has(item.label)) {
          seenItemLabels.add(item.label);
          uniqueItems.push(item);
        }
      }
      if (uniqueItems.length > 0) {
        entries.push({ id: 'genes', title: 'Gene families', items: uniqueItems.slice(0, 20) });
      }
    }
