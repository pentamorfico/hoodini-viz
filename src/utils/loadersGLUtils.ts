...
// Optimized hoods parsing (TSV with headers)
export async function parseHoodsOptimized(csvText) {
  return parseHoodsFallback(csvText);
}
...
function parseProteinMetadataFallback(str: string): Record<string, Record<string, string>> {
...
function parseHoodsFallback(str) {
  const out = [];
  if (!str) return out;
  const lines = str.split(/\r?\n/);
  ...
  for (let i = 1; i < lines.length; ++i) {
    ...
    if (!line) continue;
    const cols = line.split(/\t/);
    const start = Number(cols[startIndex]);
    const end = Number(cols[endIndex]);
    if (!isFinite(start) || !isFinite(end)) {
      // Skip invalid rows to avoid NaN lengths that break layout
      continue;
    }
    const entry = {
      hood_id: hoodIdIndex >= 0 ? cols[hoodIdIndex] : cols[seqidIndex],
      seqid: seqidIndex >= 0 ? cols[seqidIndex] : cols[hoodIdIndex],
      start,
      end
    };
