...
export function assignPaletteColors(values, paletteConfig) {
  ...
  if (type === 'sequential' && isNumericalData(validValues)) {
    ...
    console.log(`🎨 Using numerical interpolation for sequential palette: ${name}`);
    console.log(`📊 Data range: ${minValue} - ${maxValue} (${numericalValues.length} values)`);
    ...
    console.log(`🎨 Using discrete assignment for ${type} palette: ${name}`);
